...
| Latest loaded process | PID `144062`, health PASS, installed from `/src/pr-af/go`; callback `http://workforce:8007`; `PR_AF_PROVIDER=opencode`, `PR_AF_MODEL=openai/fcm`, `PR_AF_HARNESS_BIN=/src/pr-af/go/scripts/opencode-stream.sh` | PASS / CURRENT |
...
25. A separate real-repo smoke `exec_20260909_160214_zy5qk18m` was launched against immutable committed SWE-AF range `f9aec2111d084ab0204d5612f2ea00a562226ac7..0c64fe7cc4fc216f4d32d0b855015509750eb4aa` with `dry_run=true`, specifically because SWE-AF is being edited concurrently elsewhere. CURRENT status is still `running`; because touched historical `README.md` content differs from current SWE-AF HEAD, this run is diagnostic smoke only, not a strict acceptance benchmark.
26. Fresh readback recovered: PR-AF is still PID `144062`, health PASS. The decisive new localization is **QUICK_META_CONCURRENCY_STARVATION**: direct quick semantic previously passed alone (~112.7s), while full root launches all three meta lenses concurrently via `errgroup`; on the exact cloudsecurity replay, all three no-progressed and produced no artifacts. The smallest safe 80/20 repair is to serialize meta lenses only for `depth=quick`, preserving concurrent fan-out for standard/deep.

## Current blocker

The active blocker is now quick-mode meta fan-out concurrency, not change-causality logic itself. The causality repair is deterministically validated but cannot be product-accepted until a full root reaches reviewer/evidence verification. The next patch is restricted to `runMetaSelectors`: serialize the three lenses for quick depth, preserve order and fail-closed behavior, leave standard/deep concurrency unchanged, add a regression proving quick max concurrency = 1, then replay the exact cloudsecurity negative benchmark.
