...
| Maintained package | User-authorized `vps-terminal-dev` lifecycle route was implemented container-first, targeted-tested, canonicalized to `n0namer/vps-terminal` PR #144 head `87247dc2b3d9086feca9d55a59b7567d9797ce12`, loaded into DEV, and then used to run the exact `af install /src/pr-af/go --force --json`. Fresh readback: live and installed `internal/orch/phases.go` now both SHA-256 `7fc7234710046a64a40166570a3d4d2a27978bb7e6d80bb6024086a900063d00`; installed binary SHA-256 `59549708e473c7784a7c119bff1801fffd6b1ccc6f6f4288ca6e12da43322c97`. | LOADED IDENTITY MATCH |
| Runtime/load state | Fresh `/afhome/installed.yaml` reports PR-AF installed `2026-09-10T16:04:49Z`, running on port `8007`, PID `200930`, started `2026-09-10T16:04:53Z`; SDK log records successful node registration and direct local `/health` is PASS (`{"status":"ok"}`). Non-secret `/proc/200930/environ` confirms provider/model/harness/server but **does not contain `AGENT_CALLBACK_URL`**. An authenticated exact 1-second dry-run through the AgentField control plane reached dispatch but failed `agent_unreachable`: control plane attempted `http://localhost:8007/reasoners/review` and got connection refused. This is callback registration/topology drift, not budget-review evidence. | PR-AF LOCAL HEALTH PASS / CONTROL-PLANE CALLBACK BROKEN |
...
The package-lifecycle capability gap is closed. The user authorized a narrow `vps-terminal-dev` change; exact PR-AF install/start argv were added container-first, targeted-tested, canonicalized to the existing terminal branch/PR, deployed, and successfully used to install the current `/src/pr-af/go` candidate. PR-AF is now running with loaded source identity matched to the tested fail-closed code. The immediate blocker is **BUDGET_FALSE_SAFE RUNTIME ACCEPTANCE**: direct `/health` plus a bounded dry-run budget-exhaustion probe still need executed proof. A second narrow terminal probe adapter (exact local PR-AF health + bounded dry-run control-plane request; widened URLs/side effects rejected) has container-first `node_check` and targeted mediation test PASS and is being deployed; until CURRENT runtime readback shows that adapter loaded, do not claim the runtime fail-closed gate PASS. Product blocker behind it remains **QUICK_BUDGET_OVERSPEND**.
