...
The package-lifecycle and runtime-probe capability gaps are closed, and local PR-AF health is PASS. The immediate blocker is now **PR_AF_CALLBACK_TOPOLOGY_DRIFT**. The authenticated exact 1-second dry-run reached the AgentField control plane, but dispatch failed `agent_unreachable` because the control plane called `http://localhost:8007/reasoners/review`; fresh process env shows the installed PR-AF lacks `AGENT_CALLBACK_URL`. This means the fail-closed code has not yet been functionally exercised through the real control-plane→workforce callback boundary. The narrow recovery is PR-AF-only: stop the current node, restart it with exact `AGENT_CALLBACK_URL=http://workforce:8007` while preserving provider/model/harness, verify new PID/env/health/registration, then repeat the same bounded authenticated canary once. No reinstall, shared-workforce restart, or VPS reboot is justified. Product blocker behind it remains **QUICK_BUDGET_OVERSPEND**.
