...
| Runtime/load state | Fresh `/afhome/installed.yaml` reports PR-AF installed `2026-09-10T16:04:49Z`, running on port `8007`, PID `200930`, started `2026-09-10T16:04:53Z`; fresh SDK log records initialize → node.register.complete → initialize.complete. Non-secret `/proc/200930/environ` readback confirms `PR_AF_PROVIDER=opencode`, `PR_AF_MODEL=openai/fcm`, `PR_AF_HARNESS_BIN=/src/pr-af/go/scripts/opencode-stream.sh`, and `AGENTFIELD_SERVER=http://control-plane:8080`. Direct health and functional fail-closed canary remain the next runtime proof. | PR-AF RUNNING / RUNTIME ACCEPTANCE ACTIVE |
