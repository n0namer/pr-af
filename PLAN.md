...
13. direct semantic isolation `exec_20260909_115407_kcd3mxuz` reproduced the failure with compact precomputed intake/anatomy and the correct `/src/pr-af` cwd. OpenCode created the required output file and wrote a valid partial object containing `lens=SEMANTIC` and `confidence=0.95`, then made no progress until the 360s watchdog. This proves AgentField routing, PR-AF, OpenCode, model transport, repository reads, Write tool, and output-file path all work; the first failing layer is unbounded semantic investigation/completion behavior in quick mode.
14. B4 reviewer recall repair in `reviewdim.go` remains unaccepted. The next patch is restricted to quick semantic meta planning: bound repository investigation and require early completion of the full output object before further exploration. No timeout increase and no reviewer-logic change are justified.

## Current blocker

The active blocker is now narrowly localized to `meta_semantic` quick-mode completion. L2 transport/write-path is proven; semantic meta starts a correct output artifact but does not finish the required dimensions/rationale before the no-progress watchdog. Until a direct semantic canary succeeds, full XOR/clean quality runs remain EVIDENCE_MISSING rather than recall/precision failures.
