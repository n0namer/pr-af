...
| Latest independently verified loaded process | PID `144062`, health PASS after the change-causality repair was installed from `/src/pr-af/go`; callback `http://workforce:8007`; `PR_AF_PROVIDER=opencode`, `PR_AF_MODEL=openai/fcm`, `PR_AF_HARNESS_BIN=/src/pr-af/go/scripts/opencode-stream.sh`. Fresh CURRENT PID/readback is temporarily unavailable because the VPS/AgentField transport is returning Portwing/Bad-Gateway errors; do not assume this PID is still current until reread. | LAST VERIFIED / CURRENT READBACK BLOCKED |
