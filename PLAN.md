...
BMAD `bmad-testarch-test-design` / test-review re-audited the final product-evidence gate against the actual mock implementation. `go/test/e2e/run.sh` is a good **plumbing** E2E harness: it exercises the full `pr-af.review` orchestration over a seeded local 2-commit repo, records execution artifacts and harness-role counts, asserts terminal success/findings/non-empty review, and enforces zero GitHub writes. It is **not** a valid product-recall oracle in its current mode. The harness forces `PR_AF_PROVIDER=opencode` to a deterministic `test/mockcli` shim; `roleReviewDimension` synthesizes findings from scenario severities/target-file names rather than discovering defects in the seeded code, and `roleEvidenceVerifier` deterministically confirms those findings. Therefore changing the seeded defect cannot prove semantic/mechanical/systemic recall while this shim remains the reviewer. The previous statement that this harness alone was the right base for the 4-case recall matrix is superseded.
...
## Compile/vet surrogate verification + recall-route correction — 2026-09-13

BMAD test-review found a safe 80/20 way to close most of the local compiler/static-analysis evidence without bypassing mediation. On the exact restored live source, `/usr/local/go/bin/go test -vet=all ./... -run '^$' -count=1` PASSed across the module, which forces package compilation plus Go vet analysis without executing tests. A second `/usr/local/go/bin/go test ./... -run '^$' -count=1` PASSed as a compile-only package sweep, including `cmd/pr-af`; the previously recorded `/usr/local/go/bin/go test ./... -count=1` remains the fresh full-test PASS. This gives direct compile+vet+test coverage through a CURRENT-callable typed `go test` route. Exact `go build ./...` is still not directly callable, so canonical `make check` must still be reported as not-exactly-executed rather than PASS; however build/vet transport is no longer a blocker for moving to product-level review-quality evidence.

The product-level acceptance route also needs correction. The current `go/test/e2e/run.sh` cannot decide recall because its reviewer path is deterministic mockcli: `roleReviewDimension` emits scenario-driven mock findings and `roleEvidenceVerifier` marks them verified. Keep that harness for orchestration/plumbing regression only. For recall/false-positive acceptance, use a **real harness provider** (`aforge` default or real `opencode`, not `test/mockcli`) against the same local seeded repos with `dry_run=true` and zero GitHub writes. Fresh reuse-before-build probing found no ready real-provider runtime on the exact source: both `agentfield-dev-workforce` and `agentfield-dev-runtime-capture` see `/src/pr-af/go`, but neither exposes `/usr/local/bin/aforge` nor `/usr/local/bin/opencode`; previously checked adjacent registered DEV targets do not mount this PR-AF source. Direct probing of an unbound workbench container was correctly refused by operator mediation as `scope_unknown`, and AgentField capability discovery is still returning `Bad Gateway`. Therefore no real-model acceptance run is claimed yet, and creating a new validator/runtime remains out of scope unless explicitly approved.
...
## Causal real-provider eval design — 2026-09-13

BMAD `bmad-testarch-automate` plus external LLM-eval guidance were applied to the blocked product-quality gate. The key correction is to make each future real-provider run **causal**, not merely observational.

For QUICK fusion, no source mutation is needed: the public `review` input already accepts explicit `depth`, and live source proves `depth=quick` skips separate mechanical/systemic meta jobs while `depth=standard` keeps independent lenses. Therefore compare the *same seeded repo and provider* with explicit `depth=quick` versus `depth=standard`; keep model/provider/version, budget, timeout, repo snapshot, and prompt/source SHA fixed. Measure case-specific defect detection, clean-control false positives, wall time, and planner/reviewer call counts. A QUICK win requires no recall/precision regression and materially fewer meta calls; otherwise fusion is not earned.

For prompt-only OLD→NEW semantics, comparing QUICK to STANDARD would not isolate the prompt rule because both paths still share `ReviewDimension`. The discriminating experiment is an A/B source mutant on one semantic seed: current source versus a temporary mutant that removes only the `## Proposed-Diff Verification Rule` block while leaving deterministic `semanticDeltaHints` and the 241-line fallback unchanged. A stale-safe preview against live `go/internal/reasoners/reviewdim.go` SHA-256 `55410427e0cd2990e2d052a453c8dc65d2c8125eaec37fc6c649795d15bea9a3` succeeds and would produce SHA-256 `7af77fc67ec697f8f630d08bcd6b80fa927d37f6649c59007148f372d3d45f88`. Do not apply this mutant until the real-provider runtime exists and exact restore + paired execution can happen in one bounded batch.

Eval sequencing is now cost-aware and anti-overfit: first run only two smoke cases with the real provider — one semantic seeded defect and one clean control. If the harness/provider path is stable and both oracles are trustworthy, add mechanical and systemic cases. Run deterministic assertions before any judge-style grading. Do not tune on the final acceptance cases; if iteration becomes necessary, create separate dev fixtures and keep the acceptance fixtures frozen. Because LLM behavior is stochastic, a single clean 4-case sweep is directional evidence, not a statistical release benchmark; repeat only the cases that decide a preservation choice or look unstable, with provider/model/version pinned.

External skill research supports this route rather than a broad framework install: current LLM-evaluation guidance recommends dataset-before-metrics, deterministic checks before judge grading, frozen test cases, and explicit cost/latency slices; real-harness eval tooling recommends baseline comparison through the actual agent CLI and repeated runs because single stochastic runs can mislead. Applied decision: use these principles inside PR-AF's existing harness/runtime contract; do not add a new eval framework unless the native route proves insufficient.
...
## BMAD course correction — architecture functional bring-up — 2026-09-13

User clarification overrides the previous emphasis on paid acceptance experiments: the immediate project objective is to run the original PR-AF implementation and verify that the architecture's intended functions actually execute end-to-end. The previously proposed **USD 4** figure was only a conservative cap derived from the architecture's default `max_cost_usd = 2.0` multiplied by two proposed real-provider smoke runs; it is **not** a PR-AF requirement, milestone, or project budget and must not be treated as the project's blocker.

Current stage is therefore **Architecture Functional Bring-Up**, not benchmark acceptance. The source architecture defines a 7-phase adaptive review pipeline: Intake → Anatomy → Planning → Parallel Review → Review Layer (cross-reference, adversary, coverage) → deterministic synthesis/scoring → Output. Fresh live-code readback shows `Orchestrator.Run` executes Intake and Anatomy, then `runReviewPhases` drives meta planning, reviewer fan-out, streaming review layer, coverage + consistency verification, synthesis/scoring, and finally output/HITL handling. The code paths exist; the remaining job is to prove the original implementation can execute those intended functions together on the live candidate, not to invent extra benchmark infrastructure first.

Functional bring-up DoD:
1. Start the original PR-AF node/runtime from the exact live `/src/pr-af/go` candidate without GitHub-first code edits or checkout/reset.
2. Execute one full `review` request through the original pipeline with `dry_run=true` and zero GitHub writes.
3. Capture evidence that each architecture responsibility is reached: intake classification, anatomy/context, dynamic planning, reviewer execution, cross-reference/adversary/coverage or their explicit bounded no-op outcome, consistency/evidence verification, deterministic scoring/synthesis, and final structured output.
4. If any architecture responsibility is missing, broken, skipped unexpectedly, or only mocked, localize the exact code/runtime gap and fix it directly in the container; validate and restore/canonicalize through the existing SourceLoop/write-back path.
5. Only after the full original architecture is functionally alive do quality comparisons (QUICK vs STANDARD, prompt-rule A/B, seeded recall/false-positive evaluation) become the next phase.

Important distinction: mockcli may prove plumbing/wiring but cannot prove semantic reviewer quality. A real harness/provider is therefore needed eventually to prove the semantic phases behave as intended, but the project stage is first **functional completeness of the original architecture**. Any external model spend is an execution detail of the chosen provider, not the definition of success; if a paid provider is the only available route, confirm the actual spend at that point rather than carrying an artificial `$4` project gate.
