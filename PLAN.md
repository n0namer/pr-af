...
16. PR-AF Docker/runtime already owns pinned OpenCode `v1.17.15`; the smallest isolated test is to point only PR-AF at `/afhome/opencode-runtime/v1.17.15/opencode`, preserving current ProviderEnv/model config while avoiding the shared SWE wrapper. If direct semantic succeeds, continue to root XOR/clean/XOR on that exact runtime.

## Current blocker

The active blocker is the shared OpenCode wrapper buffering contract versus the AgentField SDK idle watchdog. Semantic reasoning itself has now produced the correct XOR diagnosis and complete output artifact. B4 product quality remains EVIDENCE_MISSING only because the wrapper hides child progress from the SDK. The next test is PR-AF-only raw pinned OpenCode; no shared-wrapper edit, timeout increase, or reviewer-logic expansion is authorized.
