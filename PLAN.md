...
12. corrected XOR root canary `exec_20260909_114330_ce3vdgsf` is running with the same diff plus `repo_path=/src/pr-af`; CURRENT child-process readback proves OpenCode is now invoked with `--dir /src/pr-af`. Do not classify B4 until its terminal payload is inspected.
13. B4 reviewer recall repair in `reviewdim.go` remains unaccepted until the corrected full root semantic gate runs. No further reviewer-logic change is justified before that evidence.

## Current blocker

There is no L2 meta/harness blocker. The only active B4 gate is terminal semantic evidence from the corrected full root canary using the live source context, followed by clean-negative and one XOR repeat on the same runtime identity. Historical `run_20260903_132450_lcvyp3i6` remains the known XOR miss; historical `run_20260903_131346_m6jyj65p` remains the clean-negative PASS baseline.
