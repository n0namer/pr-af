...
Source-only BMAD quick-dev + testarch-trace/test-design refinement narrowed the current live preservation surface. Against accepted baseline `1967bb...`, the high-value subset inspected in this batch is 9 files, `+578/-45`. `orch/phases.go` contains four separable behaviors: quick-meta fusion, primary-review budget fail-closed, all-finding evidence verification/drop of `verified=false`, and post-obligation-extraction budget re-check. The causality requirement itself is compact: `prompts/reviewdim.go` +9 lines and `prompts/verify.go` +4 lines. Repository-path anchoring is likewise compact: `reasoners/meta.go` +9 lines. These small contract deltas are therefore preferred preservation units over the 241-line deterministic semantic-delta fallback.

Fresh external-skill research reinforces the gate rather than changing it: verification-before-completion requires evidence that could actually go negative; code-review guidance requires callers/contracts and concrete reproductions rather than diff-only plausibility; Go mutation-testing guidance explicitly treats killed/survived mutants as test-strength evidence rather than line coverage. Apply these only as advisory support for BMAD ATDD/trace: each preservation regression must name the protected failure boundary and, where safe and bounded, prove it by killing one representative controlled mutant. No new mutation-testing dependency is justified while the canonical Go execution route is blocked.
