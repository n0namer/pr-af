...
| Maintained package | Fresh `vps-terminal-dev` readback shows `/afhome/installed.yaml` still points `pr-af.source_path=/src/pr-af/go`, desired/status `running`, port `8007`, stale recorded PID `156016`, installed at `2026-09-09T16:50:12Z`. Installed `internal/orch/phases.go` hash is `4bc518...`, while live source hash is `7fc723...`; installed code still returns `nil` on review-budget exhaustion. | DESIGN_RUNTIME_DRIFT / INSTALLED COPY STALE |
| Runtime/load state | `vps-terminal-dev` recovered scoped container visibility: workforce container `1c4f2518...` is `running` and healthy, target stats are readable, but no live PR-AF process is observed and `127.0.0.1:8007/health` refuses connection. The prior shared restart did not create a new workforce container (current container uptime ~46h). Current blocker is no longer observability; it is loading the tested source into PR-AF without broadening blast radius. | WORKFORCE HEALTHY / PR-AF STOPPED |
...
The observability outage is closed: `vps-terminal-dev` can again read target stats, container inventory, logs, and live source. The immediate blocker is now **DEV_PACKAGE_LIFECYCLE_CAPABILITY_GAP + PR-AF STOPPED**. The live source contains the tested `BUDGET_FALSE_SAFE` repair, but the installed package is byte-stale and still has the old fail-open review-budget branch. `vps-terminal-dev` currently exposes no exact typed PR-AF package install/start action: direct `exec`/`execContainer` attempts for `af install` are blocked by enforced operator mediation as `opaque_or_unknown_mutation`, while `prepareChange` reports `approval_capability_gap` because no executable generic approval adapter exists. Do not bypass this by expanding live-patch roots into `/afhome`, restarting the shared workforce, or falling back to GitHub/redeploy. Product blocker behind it remains **QUICK_BUDGET_OVERSPEND + BUDGET_FALSE_SAFE runtime acceptance**.
...
User explicitly authorized the previously gated recovery action (`Делай что надо`). A restart of the shared AgentField DEV Coolify application/workforce was requested through the authoritative Coolify control plane. The first restart call returned HTTP 504, so post-state was inspected before retry; the application still read `running:unknown` with no restart metadata. One bounded retry then returned `Deployment already queued for this commit`, proving the restart request is queued and must **not** be submitted again. Current host metrics during this queued/degraded state show CPU `100%` and RAM ~`7.66 GiB` on the 8 GiB VPS. NEXT: wait for the queued restart to settle and recover scoped container readback; then inspect PR-AF package/source/process post-state before any further install/reload. If the queued restart cannot complete and host-level Docker remains saturated, a VPS reboot would be a new major-blast recovery action and requires separate explicit approval before execution.
