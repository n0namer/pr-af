...
| Maintained package | Last verified `/afhome/installed.yaml` entry pointed `pr-af.source_path=/src/pr-af/go` at port `8007`; CURRENT package post-state after the timed-out install attempt cannot yet be reread because target file/container resolution is intermittently timing out. | LAST VERIFIED / READBACK BLOCKED |
| Runtime/load state | Last accepted runtime identity was PID `156016` with health PASS and the quick-meta serialization build. Fresh OBSERVE later found **no PR-AF process** (`pgrep` exit `1`) and `127.0.0.1:8007/health` refused connection. A subsequent scoped `af install /src/pr-af/go --force --json` attempt timed out; post-state is still unreadable. The enclosing shared AgentField DEV Coolify app `edshqtkwskg3lrczekhcmd71` is CURRENT `running:unknown`; VPS is running with continuous uptime, while Docker/Portwing reads and Coolify logs intermittently return timeout/504. Do **not** restart the shared app/host or assume PR-AF loaded until scoped post-state readback succeeds. | PR-AF NOT OBSERVED RUNNING / CONTROL READBACK DEGRADED |
...
The immediate blocker is **CONTROL_PLANE_OBSERVABILITY_DEGRADED + PR-AF NOT OBSERVED RUNNING**; the product blocker behind it remains **QUICK_BUDGET_OVERSPEND + BUDGET_FALSE_SAFE acceptance**. Fail-closed behavior is deterministic PASS in live source but not runtime-proven. The safe sequence is: on the first stable scoped readback, inspect `/afhome/installed.yaml`, `/src/pr-af` source state, PR-AF/OpenCode processes, and the timed-out install post-state → if the package is already installed and only PR-AF is absent, start **only PR-AF** with the proven callback/provider/model/harness contract → verify health and loaded identity → run a bounded fail-closed budget canary. No shared-app/host lifecycle mutation is justified by current evidence.
